...
import { addAddiction, updateAddiction } from '../store/slices/addictionsSlice';
...
export default function AddVicePage() {
  ...
  const [form, setForm] = useState({
    ...
    lastRelapseDate: existingAddiction?.lastRelapseDate
      ? existingAddiction.lastRelapseDate.substring(0, 16)
      : '',
    dailySpending: existingAddiction?.dailySpending || '',
    currency: existingAddiction?.currency || user?.currency || 'INR',
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    if (!selected) {
      return toast.error('Please select a vice type');
    }
    setLoading(true);
    try {
      const payload = {
      ...
      if (isEditing) {
        await dispatch(updateAddiction({ id, data: payload })).unwrap();
        toast.success('Vice updated successfully! 📝');
      } else {
        await dispatch(addAddiction(payload)).unwrap();
        toast.success('Vice added! Your timer starts now 🎉');
      }
      navigate('/dashboard');
    } catch (err) {
      toast.error(typeof err === 'string' ? err : 'Failed to add vice');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="page max-w-2xl">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <h1 className="font-display font-bold text-3xl text-white mb-2">
          {isEditing ? 'Edit Your Vice' : 'Track a Vice'}
        </h1>
        <p className="text-slate-400 mb-8">
          {isEditing
            ? 'Update your vice details below.'
            : "Select what you want to quit and we'll start counting your progress."}
        </p>
        ...
        <form onSubmit={onSubmit} className="glass p-6 space-y-5">
          ...
          <div>
            ...
            <input
              id="vice-relapse-date"
              type="datetime-local"
              name="lastRelapseDate"
              value={form.lastRelapseDate}
              onChange={onChange}
              className="input"
            />
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-2">
              Your "Why" <span className="text-slate-500">(optional)</span>
            </label>
            <textarea
              id="vice-note"
              name="motivationalNote"
              value={form.motivationalNote}
              onChange={onChange}
