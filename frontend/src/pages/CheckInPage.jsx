...
import { fetchRecentCheckIns, addCheckIn } from '../store/slices/checkInsSlice';
...
export default function CheckInPage() {
  ...
  const { items: addictions } = useSelector((state) => state.addictions);
  const { recent: recentCheckIns, submitting: loading } = useSelector((state) => state.checkIns);
  ...
  useEffect(() => {
    ...
    dispatch(fetchRecentCheckIns(5));
  }, [dispatch]);

  useEffect(() => {
    if (addictions.length > 0 && !form.addictionId) {
      setForm((f) => ({ ...f, addictionId: addictions[0]._id }));
    }
  }, [addictions, form.addictionId]);
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    if (!form.addictionId) {
      return toast.error('Select an addiction to check in for');
    }
    ...
    try {
      await dispatch(
        addCheckIn({
          ...form,
          urgeMeter: parseInt(form.urgeMeter, 10),
        })
      ).unwrap();
      ...
      toast.error(typeof err === 'string' ? err : 'Failed to save check-in');
    }
  };
    ...
    <div className="page max-w-3xl">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-slate-400 mb-8">
          How are you feeling right now? Tracking urges helps you understand your patterns.
        </p>
        ...
        <form onSubmit={onSubmit} className="glass p-6 space-y-6 mb-8">
          ...
          <div>
            ...
            {addictions.length === 0 ? (
              <p className="text-slate-500 text-sm">
                ...
                <Link to="/add-vice" className="text-teal-400">
                  Add one first
                </Link>
                .
              </p>
              ...
              <select
                id="checkin-addiction"
                name="addictionId"
                value={form.addictionId}
                onChange={onChange}
                className="input"
              >
                {addictions.map((a) => (
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-2">
                <span className={`font-display font-black text-3xl ${urgeColor(form.urgeMeter)}`}>
                  {form.urgeMeter}
                </span>
                <span className={`text-sm ${urgeColor(form.urgeMeter)}`}>
                  {urgeLabel(parseInt(form.urgeMeter, 10))}
                </span>
              </div>
            </div>
            <div className="relative">
              <input
                ...
                min={1}
                max={10}
                value={form.urgeMeter}
                onChange={onChange}
                className="w-full h-2 rounded-full appearance-none cursor-pointer"
                style={{
                  background: `linear-gradient(to right, #14b8a6 0%, #f59e0b 50%, #ef4444 100%)`,
                }}
              />
            ...
            <div className="flex gap-2 flex-wrap">
              {MOODS.map((m) => (
                ...
                >
                  <m.icon
                    size={22}
                    className={form.mood === m.val ? 'text-teal-400' : 'text-slate-500'}
                  />
                  <span>{m.label}</span>
                </button>
              ))}
          ...
          <button
            id="checkin-submit"
            type="submit"
            disabled={loading || addictions.length === 0}
              ...
              {recentCheckIns.map((c) => (
                <div key={c._id} className="glass p-4 flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-slate-200 capitalize">
                        {c.addictionId?.viceName || 'Unknown'}
                      </p>
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <p className="text-xs text-slate-600 capitalize">{c.mood}</p>
                  </div>
                </div>
              ))}
            </div>
