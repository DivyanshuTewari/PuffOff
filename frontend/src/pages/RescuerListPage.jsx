...
import { Activity, Plus, ChevronRight, Flame, TrendingDown, Trash2 } from 'lucide-react';
import toast from 'react-hot-toast';

import { fetchRescuerPlans, deleteRescuerPlan } from '../store/slices/rescuerSlice';
...
export default function RescuerListPage() {
  ...
  const executeDelete = async () => {
    ...
    try {
      await dispatch(deleteRescuerPlan(planToDelete)).unwrap();
  ...
  const loading = (plansLoading || addictionsLoading) && plans.length === 0 && addictions.length === 0;
            ...
            >
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 justify-center px-4 py-2.5 rounded-xl border border-red-500/50 bg-red-500/20 text-red-400 hover:bg-red-500/30 font-medium transition-colors"
                >
                  Delete
                </button>
              </div>
                  ...
                  {plans.map((plan) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                            <span className="font-semibold text-white capitalize">
                              {plan.addictionId?.customName ||
                                plan.addictionId?.viceName?.replace('_', ' ') ||
                                'Vice'}
                            </span>
                            <span
                              className={`text-xs px-2 py-0.5 rounded-full border font-medium ${phase.bg} ${phase.color}`}
                            >
                          ...
                          <div className="flex items-center gap-4 text-sm flex-wrap">
                            <span className="text-slate-400">
                              Target: <span className="text-white font-semibold">{plan.currentDailyTarget}</span>/
                              {plan.baselineDaily} {plan.unit}
                            </span>
                            {consumed !== null && (
                              <span
                                className={
                                  consumed <= plan.currentDailyTarget ? 'text-teal-400' : 'text-red-400'
                                }
                              >
                        ...
                        <ChevronRight
                          size={16}
                          className="text-slate-600 group-hover:text-teal-400 transition-colors shrink-0 ml-1"
                        />
                      </Link>
                    );
                  })}
            ...
            {plans.length === 0 && addictions.length === 0 && (
              ...
              >
                ...
                <p className="text-slate-400 text-sm mb-6">
                  Add a vice first, then come back to set up your Rescuer plan.
                </p>
