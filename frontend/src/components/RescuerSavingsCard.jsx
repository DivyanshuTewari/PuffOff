...
export default function RescuerSavingsCard({ plan }) {
  ...
  const { logs, weeklySchedule, pricePerUnit, currency, unit, baselineDaily, baselineQuantity, frequency } = plan;
  const sym = getCurrencySymbol(currency);

  const dailyBaseline = frequency === 'weekly' ? (baselineQuantity || baselineDaily) / 7 : (baselineQuantity || baselineDaily);
  const weeklyBaseline = frequency === 'weekly' ? (baselineQuantity || baselineDaily) : dailyBaseline * 7;
  ...
  const skippedToday = Math.max(0, dailyBaseline - todayConsumed);
  ...
  const weeklyExpectedAtThisPoint = (weeklyLogs.length / 7) * weeklyBaseline;
  const weeklySkipped = Math.max(0, weeklyExpectedAtThisPoint - weeklyConsumed);
  ...
  const fmtSkippedToday = skippedToday % 1 === 0 ? skippedToday : skippedToday.toFixed(1);
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="bg-white/4 rounded-xl p-3 border border-white/5">
          ...
          <p className="text-slate-500 text-xs mt-0.5">{fmtSkippedToday} {unit} reduced</p>
        </div>
        <div className="bg-white/4 rounded-xl p-3 border border-white/5">
          ...
          {equivalent && <p className="text-slate-500 text-xs mt-0.5">≈ {equivalent}</p>}
        </div>
      </div>
      ...
      {savedToday > 0 && (
        <div className="flex items-start gap-2 bg-green-500/8 rounded-xl p-3 border border-green-500/15">
          ...
          <p className="text-green-300 text-xs leading-relaxed">
            By reducing {fmtSkippedToday} {unit} today, you've preserved {sym}{savedToday.toFixed(0)}.
